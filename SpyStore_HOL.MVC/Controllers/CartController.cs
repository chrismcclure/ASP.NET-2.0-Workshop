...
namespace SpyStore_HOL.MVC.Controllers
{
        ...
        {
            private readonly IShoppingCartRepo _shoppingCartRepo; private readonly ICustomerRepo _customerRepo; private readonly IProductRepo _productRepo; readonly MapperConfiguration _config = null; public CartController(IShoppingCartRepo shoppingCartRepo, ICustomerRepo customerRepo, IProductRepo productRepo)
            {
                _shoppingCartRepo = shoppingCartRepo; _customerRepo = customerRepo; _productRepo = productRepo; _config = new MapperConfiguration(cfg =>
                { cfg.CreateMap<AddToCartViewModel, ShoppingCartRecord>().AfterMap((s, t) => { t.Id = 0; t.TimeStamp = null; }); cfg.CreateMap<CartRecordViewModel, ShoppingCartRecord>(); cfg.CreateMap<CartRecordWithProductInfo, CartRecordViewModel>(); cfg.CreateMap<ProductAndCategoryBase, AddToCartViewModel>(); });
            }

        [HttpGet] public IActionResult Index(int customerId) { ViewBag.Title = "Cart"; ViewBag.Header = "Cart"; var cartItems = _shoppingCartRepo.GetShoppingCartRecords(customerId); var customer = _customerRepo.Find(customerId); var mapper = _config.CreateMapper(); var viewModel = new CartViewModel { Customer = customer, CartRecords = mapper.Map<IList<CartRecordViewModel>>(cartItems) }; return View(viewModel); }

        [HttpGet("{productId}")] public IActionResult AddToCart(int customerId, int productId, bool cameFromProducts = false) { ViewBag.CameFromProducts = cameFromProducts; ViewBag.Title = "Add to Cart"; ViewBag.Header = "Add to Cart"; ViewBag.ShowCategory = true; var prod = _productRepo.GetOneWithCategoryName(productId); if (prod == null) return NotFound(); var mapper = _config.CreateMapper(); var cartRecord = mapper.Map<AddToCartViewModel>(prod); cartRecord.Quantity = 1; return View(cartRecord); }
        ...
        {
            if (!ModelState.IsValid) return View(item); try
            {
